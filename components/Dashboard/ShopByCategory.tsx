    ...
    return (
        <div className=" flex flex-col gap-5 items-center justify-center py-2 px-[5%]">
            <div className="w-full">
                <h2 className="text-2xl font-Abyssinica font-thin text-left  w-max p-2 text-white rounded-t-md">Shop by Category</h2>
            </div>
            <div className=" grid grid-cols-2 justify-center gap-2 md:grid-cols-3 lg:grid-cols-6 xl:grid-cols-6 2xl:grid-cols-6">
                {categoryImage.map((src, index) => (
                    <div key={index} className="relative flex justify-center">
                        <img onClick={() => SelectCategories(src.title)} className="cursor-pointer " key={index} src={src.src} alt="" id='custom-image' />
                        <label onClick={() => SelectCategories(src.title)} className="absolute flex items-center justify-center text-center bg-black/30 w-full h-[15%] text-white p-1  cursor-pointer transition-all duration-200 ease-in-out" id='custom-label'>{src.title}</label>
                    </div>
